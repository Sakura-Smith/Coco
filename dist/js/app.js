function SnowStorm() {
    ...
    this.vMaxY = 4,
    this.zIndex = 0;
    var d = "undefined" == typeof window.attachEvent ?
    function(a, b, c) {
    ...
    this.randomizeWind = function() {
        if (s = b(a(f.vMaxX, .2)), t = a(f.vMaxY, .2), this.flakes) for (var c = 0; c < this.flakes.length; c++) this.flakes[c].active && this.flakes[c].setVelocities()
    },
    this.scrollHandler = function() {
        if (r = f.flakeBottom ? 0 : parseInt(window.scrollY || document.documentElement.scrollTop || document.body.scrollTop, 10), isNaN(r) && (r = 0), !x && !f.flakeBottom && f.flakes) for (var a = f.flakes.length; a--;) 0 === f.flakes[a].active && f.flakes[a].stick()
    },
    this.resizeHandler = function() {
        window.innerWidth || window.innerHeight ? (o = window.innerWidth - (h ? 2 : 16) - f.flakeRightOffset, q = f.flakeBottom ? f.flakeBottom: window.innerHeight) : (o = (document.documentElement.clientWidth || document.body.clientWidth || document.body.scrollWidth) - (h ? 0 : 8) - f.flakeRightOffset, q = f.flakeBottom ? f.flakeBottom: document.documentElement.clientHeight || document.body.clientHeight || document.body.scrollHeight),
        p = parseInt(o / 2, 10)
    },
    this.resizeHandlerAlt = function() {
        o = f.targetElement.offsetLeft + f.targetElement.offsetWidth - f.flakeRightOffset,
        q = f.flakeBottom ? f.flakeBottom: f.targetElement.offsetTop + f.targetElement.offsetHeight,
        p = parseInt(o / 2, 10)
    },
    ...
    this.SnowFlake = function(b, c, d, e) {
        ...
        this.stick = function() {
            n || g.targetElement != document.documentElement && g.targetElement != document.body ? f.o.style.top = q + r - g.flakeHeight + "px": g.flakeBottom ? f.o.style.top = g.flakeBottom + "px": (f.o.style.display = "none", f.o.style.top = "auto", f.o.style.bottom = "0px", f.o.style.position = "fixed", f.o.style.display = "block")
        },
        this.vCheck = function() {
            f.vX >= 0 && f.vX < .2 ? f.vX = .2 : f.vX < 0 && f.vX > -.2 && (f.vX = -.2),
            f.vY >= 0 && f.vY < .2 && (f.vY = .2)
        },
        this.move = function() {
            var a = f.vX * u;
            f.x += a,
            f.y += f.vY * f.vAmp,
            f.x >= o || o - f.x < g.flakeWidth ? f.x = 0 : 0 > a && f.x - g.flakeLeftOffset < 0 - g.flakeWidth && (f.x = o - g.flakeWidth - 1),
            f.refresh();
            var b = q + r - f.y;
            b < g.flakeHeight ? (f.active = 0, g.snowStick ? f.stick() : f.recycle()) : (g.useMeltEffect && f.active && f.type < 3 && !f.melting && Math.random() > .998 && (f.melting = !0, f.melt()), g.useTwinkleEffect && (f.twinkleFrame ? (f.twinkleFrame--, f.o.style.visibility = f.twinkleFrame && f.twinkleFrame % 2 === 0 ? "hidden": "visible") : Math.random() > .9 && (f.twinkleFrame = parseInt(20 * Math.random(), 10))))
        },
        this.animate = function() {
            f.move()
        },
        this.setVelocities = function() {
            f.vX = s + a(.12 * g.vMaxX, .1),
            f.vY = t + a(.12 * g.vMaxY, .1)
        },
        this.setOpacity = function(a, b) {
            return y ? void(a.style.opacity = b) : !1
        },
        this.melt = function() {
            g.useMeltEffect && f.melting && f.meltFrame < f.meltFrameCount ? (f.meltFrame++, f.setOpacity(f.o, f.meltFrames[f.meltFrame]), f.o.style.fontSize = f.fontSize - f.fontSize * (f.meltFrame / f.meltFrameCount) + "px", f.o.style.lineHeight = g.flakeHeight + 2 + .75 * g.flakeHeight * (f.meltFrame / f.meltFrameCount) + "px") : f.recycle()
        },
    ...
    this.snow = function() {
        ...
        d = 0,
        e = null,
        g = f.flakes.length; g--;) 1 == f.flakes[g].active ? (f.flakes[g].move(), b++) : 0 === f.flakes[g].active ? c++:d++,
    ...
    this.createSnow = function(b, c) {
        for (var d = 0; b > d; d++) f.flakes[f.flakes.length] = new f.SnowFlake(f, parseInt(a(w), 10)),
        (c || d > f.flakesMaxActive) && (f.flakes[f.flakes.length - 1].active = -1);
...
$(function() {
    ...
    e = $(".loading-mask"),
    f = ISPRO ? "dist": "src";
    d.imgs = [f + "/images/car.png", f + "/images/bg_bottom.png", f + "/images/sprite_bigSnow.png", f + "/images/stars.png", f + "/images/tree.png", f + "/images/bag.png", f + "/images/sprite_gifts.png", f + "/images/glove.png", f + "/images/banner.png"],
    d.loadedPer = 0,
    ...
    d.doLoadImg = function(a) {
        ...
        b.onload = function() {
            ...
            e.find(".progres-bar").css("width", d.loadedPer + "%").one(c,
            function() {
                parseInt(d.loadedPer) >= 99 && (e.find(".loading-text").text("加载完成"), $("#j-playBgMusic").fadeIn(200))
            })
        }
    };
